...
// [DIAGNOSTIC black-box] Any crash inside a listener is appended here with a
// full stack; a MOUNT marker is written once per apply() so we can verify
// which code generation the server actually loaded.
const CRASH_LOG = '/tmp/two-stage-promotion-crash.log'
function crashMark(line) {
  try { appendFileSync(CRASH_LOG, `[${new Date().toISOString()}] ${line} pid=${process.pid}\n`) } catch {}
}
function crashLine(tag, err) {
  return `[${new Date().toISOString()}] ${tag} pid=${process.pid}\n${(err && err.stack) || String(err)}\n\n`
}
function withCrashLog(tag, fn) {
  return function guarded(...args) {
    try {
      const out = fn.apply(this, args)
      if (out !== null && typeof out === 'object' && typeof out.then === 'function') {
        return out.then(undefined, (err) => {
          try { appendFileSync(CRASH_LOG, crashLine(tag, err)) } catch {}
          throw err
        })
      }
      return out
...
export function apply(ctx, config) {
  ...
  ctx.on('session/event', withCrashLog('session/event', (session, event) => {
    if (event.type === 'compaction/end') {
  ...
  ctx.on('system-prompt/assemble', withCrashLog('system-prompt/assemble', async (_assembly, context, next) => {
  ...
  ctx.on('agent/pre-step', withCrashLog('agent/pre-step', async (payload, next) => {
